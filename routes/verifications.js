...
router.get('/verificationphone',async (req,res)=>{
    const {phone} =req.headers;
    await user.findOne({phone})? res.send(true):res.status(401).send(false);
});
router.route('/:phone').get((req, res) =>{
    ...
    ! req.err ? res.send(`${token}`):res.status(401).send("phone not found");
      ...
      await user.findByIdAndUpdate({"_id":req.user._id},{"password":bcrypt.hashSync(req.body.password,Number(process.env.SALT))})
